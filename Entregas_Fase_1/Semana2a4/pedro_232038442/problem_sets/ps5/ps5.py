...
def filter(pixels_list, color):
    ...
    for p in pixels_list:
        mul = matrix_multiply(m, p)
        res.append((int(mul[0]), int(mul[1]), int(mul[2])))
    return res
...
def extract_end_bits(num_end_bits, pixel):
    ...
    if isinstance(pixel, int):
        ...
        return tuple(p % mod for p in pixel)
...
def reveal_color_image(filename):
    ...
    hidden_pixels = []
    for p in pixels:
        extr = extract_end_bits(3, p)
        hidden_pixels.append(tuple((val * 255) // 7 for val in extr))
...
def main():
    ...
    # im.show()
    im.save('image_15_none.png')

    red_filtered_pixels = filter(pixels, 'red')
    im2 = pix_to_img(red_filtered_pixels, (width, height), 'RGB')
    # im2.show()
    im2.save('image_15_red.png')

    # Uncomment the following lines to test part 2
    im_bw = reveal_image('hidden1.bmp')
    # im_bw.show()
    im_bw.save('hidden1_unhidden.bmp')

    im_rgb = reveal_image('hidden2.bmp')
    # im_rgb.show()
    im_rgb.save('hidden2_unhidden.bmp')
