...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    for pad in pads:
        ...
        words = text.split(' ')
        valid_words = sum(1 for w in words if is_word(word_list, w))
...
def decode_story():
    ...
    '''
    ciphertext_str = get_story_string()
    pads = get_story_pads()
    enc_msg = ps4b.EncryptedMessage(ciphertext_str)
    best_msg = decrypt_message_try_pads(enc_msg, pads)
    return best_msg.get_text()
