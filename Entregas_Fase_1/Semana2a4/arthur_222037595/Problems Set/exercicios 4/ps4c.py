...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    for pad in pads:
        decifrado = ciphertext.decrypt_message(pad)
        texto = decifrado.get_text()

        # conta quantas palavras validas em ingles apareceram
        palavras = texto.split()
        cont = sum(1 for p in palavras if is_word(lista_palavras, p))

        # em caso de empate fica com o ultimo (>=)
        if cont >= maior_cont:
            maior_cont = cont
            melhor = decifrado
...
def decode_story():
    ...
    '''
    historia_cifrada = ps4b.EncryptedMessage(get_story_string())
    pads = get_story_pads()
    resultado = decrypt_message_try_pads(historia_cifrada, pads)
