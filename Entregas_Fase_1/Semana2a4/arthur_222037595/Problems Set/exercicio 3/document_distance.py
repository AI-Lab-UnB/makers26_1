...
def text_to_list(input_text):
    ...
    """
    # split ja cuida de espacos multiplos e quebras de linha
...
def get_letter_frequencies(word):
    ...
    """
    # palavra eh iteravel de caracteres, entao da pra reusar get_frequencies
...
def calculate_similarity_score(freq_dict1, freq_dict2):
    ...
    """
    # uniao de todas as chaves dos dois dicionarios
    todas_chaves = set(freq_dict1.keys()) | set(freq_dict2.keys())

    diff = 0
    total = 0

    for chave in todas_chaves:
        f1 = freq_dict1.get(chave, 0)
        f2 = freq_dict2.get(chave, 0)
        diff += abs(f1 - f2)
        total += f1 + f2
...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    """
    # soma as frequencias dos dois dicionarios
    combinado = {}
    for palavra, cont in freq_dict1.items():
        combinado[palavra] = combinado.get(palavra, 0) + cont
    for palavra, cont in freq_dict2.items():
        combinado[palavra] = combinado.get(palavra, 0) + cont

    maior = max(combinado.values())

    # em caso de empate retorna lista ordenada alfabeticamente
    return sorted([p for p, c in combinado.items() if c == maior])
...
def get_tf(file_path):
    ...
    * Think about how we can use get_frequencies from earlier
    ...
    freq = get_frequencies(palavras)
    total = len(palavras)

    tf = {}
    for palavra, cont in freq.items():
        tf[palavra] = cont / total
    return tf
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    for path in file_paths:
        texto = load_file(path)
        # palavras unicas do documento
        unicas = set(text_to_list(texto))
        for palavra in unicas:
            contagem_docs[palavra] = contagem_docs.get(palavra, 0) + 1

    idf = {}
    for palavra, n_docs in contagem_docs.items():
        idf[palavra] = math.log10(total_docs / n_docs)
    return idf
...
def get_tfidf(tf_file_path, idf_file_paths):
    ...
    resultado = []
    for palavra, val_tf in tf.items():
        val_idf = idf.get(palavra, 0)
        resultado.append((palavra, val_tf * val_idf))

    # ordena por score crescente, empate resolve pela ordem alfabetica
