...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    for pad in pads:
        ...
        valid_count = sum(1 for word in words if is_word(word_list, word))
        # Use >= to return the LAST pad that ties for maximum
        if valid_count >= max_words:
            max_words = valid_count
...
def decode_story():
    ...
    pads = get_story_pads()
    best_message = decrypt_message_try_pads(ciphertext, pads)
    return best_message.get_text()
