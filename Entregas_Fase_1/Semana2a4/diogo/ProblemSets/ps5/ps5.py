...
def filter(pixels_list, color):
    ...
    """
    transf_matrix = make_matrix(color)
    
    result = []
    
    #print(f"DEBUG: pixel_tuple = {pixels_list}, type = {type(pixels_list)}") # Adicione isso
    
    
    for pixel_tuple in pixels_list:
        result.append( tuple(map(int, (matrix_multiply(transf_matrix, pixel_tuple)))))
...
def extract_end_bits(num_end_bits, pixel):
    ...
    """
    if( isinstance(pixel, int) ): # Return integer
        result = 0
        add_num = 1
...
def reveal_bw_image(filename):
    ...
    for pixel in img:
        bit = extract_end_bits(1, pixel)
        new_img.append(bit * 255)
        
    return pix_to_img(new_img, size, 'L')
...
def reveal_color_image(filename):
    ...
    for pixel_tuple in img:
        bit = extract_end_bits(3, pixel_tuple)
    ...
    return pix_to_img(new_img, size, 'RGB')
