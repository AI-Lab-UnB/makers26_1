...
def filter(pixels_list, color):
    ...
    for pixel in pixels_list:
        ...
        new_rgb = matrix_multiply(matrix, [r, g, b])
        new_pixel = tuple(min(255, max(0, int(val))) for val in new_rgb)
...
def reveal_bw_image(filename):
    ...
    for pixel in pixels:
        ...
        new_pixels.append(bit * 255)
    im = Image.open(filename)
    return pix_to_img(new_pixels, im.size, 'L')
...
def reveal_color_image(filename):
    ...
    new_pixels = []
    for pixel in pixels:
        end_bits = extract_end_bits(3, pixel)
        new_pixel = tuple(val * 255 // 7 for val in end_bits)
        new_pixels.append(new_pixel)
    im = Image.open(filename)
    return pix_to_img(new_pixels, im.size, 'RGB')
