...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    return sorted([word for word, freq in combined.items() if freq == max_freq])
...
def get_tf(file_path):
    ...
    * Think about how we can use get_frequencies from earlier
    ...
    return {word: freq / total_words for word, freq in freq_dict.items()}
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    doc_count = {}  # how many documents contain each word
 
    for file_path in file_paths:
        text = load_file(file_path)
        words = set(text_to_list(text))  # set: conta cada palavra só uma vez por doc
        for word in words:
            doc_count[word] = doc_count.get(word, 0) + 1
 
    return {word: math.log10(total_docs / count) for word, count in doc_count.items()}
