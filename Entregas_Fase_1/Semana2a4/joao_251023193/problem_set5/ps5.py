...
def img_to_pix(filename):
    ...
    #opens and identifies the given image
    img = Image.open(filename)

    #return contents of this image as a sequence containing pixels values
...
def filter(pixels_list, color):
    ...
    #create a color matrix 1x3
    color_matrix = make_matrix(color)

    for pixel in pixels_list:
        
        #color matrix * pixel for the final pixel
        new_pixel_matrix = matrix_multiply(color_matrix, pixel)

        final_new_pixel = (int(new_pixel_matrix[0]), int(new_pixel_matrix[1]), int(new_pixel_matrix[2]))

        new_pixels_list.append(final_new_pixel)
...
def extract_end_bits(num_end_bits, pixel):
    ...
    if type(pixel) == int:
        ...
        new_pixel = pixel % (2**num_end_bits)
        ...
        for number in pixel:
            ...
            new_pixel = number % (2**num_end_bits)
            temp_list.append(new_pixel)

        new_pixel_tuple = tuple(temp_list)

        return new_pixel_tuple  
...
def reveal_bw_image(filename):
    ...
    for pixel in pixels_list:
        ...
        pixel_reveal = extract_end_bits(1, pixel)

        new_pixel = pixel_reveal * 255

        list_reveal_img.append(new_pixel)

    

    hidden_img = pix_to_img(list_reveal_img, img_size, 'L')
...
def reveal_color_image(filename):
    ...
    for pixel in pixel_list:
        ...
        reveal_pixel = extract_end_bits(3, pixel)

        new_pixel = (reveal_pixel[0] * 255 // 7, reveal_pixel[1] * 255 // 7,reveal_pixel[2] * 255 // 7)

        reveal_img_list.append(new_pixel)

    
    hidden_img = pix_to_img(reveal_img_list, img_size, 'RGB')
