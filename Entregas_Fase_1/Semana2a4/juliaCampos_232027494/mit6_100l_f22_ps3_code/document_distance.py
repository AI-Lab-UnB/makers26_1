...
def get_frequencies(input_iterable):
    ...
    """
    verification = input_iterable
    final = {}

    for i in verification:
        if i not in final:
            quantidade = verification.count(i)
            final[i] = quantidade
...
def get_letter_frequencies(word):
    ...
    verification = word
    final = get_frequencies(verification)
...
def calculate_similarity_score(freq_dict1, freq_dict2):
    ...
    for lett, freq_l1 in freq_dict1.items():
        if lett in freq_dict2:
            freq_l2 = freq_dict2[lett]
            delta += abs(freq_l1 - freq_l2)
            sigma += freq_l1 + freq_l2
        else:
            delta += freq_l1
            sigma += freq_l1

    for lett, freq_l2 in freq_dict2.items():
        if lett not in freq_dict1:
            delta += freq_l2
            sigma += freq_l2
...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    for lett,freq_l1 in freq_dict1.items():
        if lett in freq_dict2:
            freq_t = freq_dict2[lett] + freq_l1

        else:
            freq_t = freq_l1

        final_list[lett] = freq_t
        
        
    
    for lett,freq_l2 in freq_dict2.items():
        if lett not in freq_dict1:
            freq_t = freq_dict2[lett]
    ...
    higher_word = dict(sorted(final_list.items()))
    higher_word = dict(sorted(final_list.items(), key=lambda item:item[1], reverse=True))
    return higher_word.keys()
...
def get_tf(file_path):
    ...
    * Think about how we can use get_frequencies from earlier
    ...
    word_counts = get_frequencies(words_list)
    tf = {}
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    for path in file_paths:
        words_in_doc = set(text_to_list(load_file(path)))
        word_doc_counts.update(words_in_doc)
        
    idf = {}
    for word, count in word_doc_counts.items():
        idf[word] = math.log10(total_files / count)
    return idf
...
def get_tfidf(tf_file_path, idf_file_paths):
    ...
    for word, tf_value in tf_dict.items():
        ...
        tfidf_list.append((word, tfidf_score))
        
    sorted_tfidf = sorted(tfidf_list, key=lambda x: (x[1], x[0]))
