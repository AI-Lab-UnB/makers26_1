...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    for pad in pads:
        message = ciphertext.decrypt_message(pad)
        decifred_text = message.get_text()
        message_list = decifred_text.split()
        count = 0
...
def decode_story():
    ...
    history = get_story_string()
    pads = get_story_pads()

    cif_histr = ps4b.EncryptedMessage(history)
    best_object = decrypt_message_try_pads(cif_histr,pads)
    
    return best_object.get_text()
