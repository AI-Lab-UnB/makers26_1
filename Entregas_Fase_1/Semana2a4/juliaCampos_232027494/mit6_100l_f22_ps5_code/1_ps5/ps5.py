...
def filter(pixels_list, color):
    ...
    matrix_to_use = make_matrix(color)
    list_final = []
    
    for i in pixels_list:
        value = matrix_multiply(matrix_to_use,i)
        val_int = []

        for j in value:
            val_int.append(int(j))


        pixel_tuple = tuple(val_int)
        list_final.append(pixel_tuple)
...
def extract_end_bits(num_end_bits, pixel):
    ...
    if type(pixel) == tuple:
        ...
        for i in pixel:
            last_bits = i % (2**num_end_bits)
            list_to_return.append(last_bits)

        return tuple(list_to_return)
    
    else:
        last_bits = pixel % (2**num_end_bits)
        return last_bits
...
def reveal_bw_image(filename):
    ...
    for i in pix:
        ...
        if extract_end_bits(1,i):
            to_convert.append(255)
        else:
            to_convert.append(0)

    return pix_to_img(to_convert, (width,height), 'L')
...
def reveal_color_image(filename):
    ...
    pix_tuple = img_to_pix(filename)

    for i in pix_tuple:
        to_convert = []
        value = extract_end_bits(3,i)

        for j in value:
            to_convert.append(j*36)
        convert_final.append(tuple(to_convert))

    return pix_to_img(convert_final, (width,height), 'RGB')
