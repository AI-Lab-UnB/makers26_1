...
def filter(pixels_list, color):
    ...
    for pixel in pixels_list:
        ...
        new_pixel_float = matrix_multiply(matrix_color, pixel)
        
        new_pixel_final = (int(new_pixel_float[0]), int(new_pixel_float[1]), int(new_pixel_float[2]))

        list_new_pixels.append(new_pixel_final)

    return list_new_pixels
...
def extract_end_bits(num_end_bits, pixel):
    ...
    if type(pixel) is tuple :
        ...
        for num in pixel:
            ...
            new_pixel = num % (2**num_end_bits)

            new_pixel_list.append(new_pixel)

        
        new_pixel_final = tuple(new_pixel_list)

        return new_pixel_final
...
def reveal_bw_image(filename):
    ...
    list_pixel = img_to_pix(filename)
    ...
    for pixel in list_pixel:
        ...
        reveal_pixel = extract_end_bits(1 , pixel)

        new_pixel = reveal_pixel * 255

        list_reveal_img.append(new_pixel)

    # print(list_reveal_img)

    hidden_img = pix_to_img(list_reveal_img, size , 'L')
...
def reveal_color_image(filename):
    ...
    hidden_img = pix_to_img(list_reveal_img, size , 'RGB')
...
def main():
    ...
    # draw_kerb("image_15.png", "timthebeaver")
