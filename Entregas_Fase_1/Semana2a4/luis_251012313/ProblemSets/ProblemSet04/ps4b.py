...
class Message(object):
    ...
    def apply_pad(self, pad):
        ...
        '''
        return ''.join( [ (self.shift_char(self.text[i], pad[i])) for i in range(len(pad)) ] )
...
class EncryptedMessage(Message):
    def __init__(self, input_text):
        ...
        '''
        return super().__init__(input_text)
    ...
    def decrypt_message(self, pad):
        ...
        '''
        text = self.get_text()
        return PlaintextMessage( "".join([self.shift_char(text[i], -pad[i]) for i in range(len(text))]), pad )