...
def get_story_string():
    ...
    f = open(os.path.join(base_dir, "story.txt"), "r")
    story = str(f.read())
    f.close()
    return story[:-1]
...
def get_story_pads():
    base_dir = os.path.dirname(__file__)
    with open(os.path.join(base_dir, "pads.txt")) as json_file:
...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    '''
    base_dir = os.path.dirname(__file__)
    word_list = load_words(os.path.join(base_dir, "words.txt"))
    list = []

    for pad in pads:
        sum = 0
        complete_text = ciphertext.decrypt_message(pad).get_text().split()
    ...
    for i in range(len(list) - 1, -1, -1):
        if list[i] == high:
            return ciphertext.decrypt_message(pads[i])    
