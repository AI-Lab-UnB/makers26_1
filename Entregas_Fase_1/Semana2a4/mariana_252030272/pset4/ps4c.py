...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    for pad in pads:
        soma = 0
        complete_text = message.decrypt_message(pad).get_text().split()
        for palavra in complete_text:
            if is_word(word_list, palavra):
                soma += 1
        nota.append(int(soma))
        

    maximo = max(nota)
    for i in range(len(nota) - 1, -1, -1):
        if nota[i] == maximo:
            return message.decrypt_message(pads[i])
...
def decode_story():
    ...
    return decrypt_message_try_pads(ps4b.EncryptedMessage(string), pads)
