...
def filter(pixels_list, color):
    ...
    for pixel in pixels_list:
        result = matrix_multiply(matrix, pixel)
        new_pixel = tuple(min(255, max(0, int(round(v)))) for v in result)
...
def extract_end_bits(num_end_bits, pixel):
    ...
    if isinstance(pixel, tuple):
        return tuple(p % (2 ** num_end_bits) for p in pixel)
    else:
...
def reveal_bw_image(filename):
    ...
    img = Image.open(filename)
    size = img.size
    # Extract 1 LSB and rescale: 0 -> 0, 1 -> 255
...
def reveal_color_image(filename):
    ...
    img = Image.open(filename)
    size = img.size
    # Extract 3 LSBs per channel and rescale from range [0,7] to [0,255]
    hidden_pixels = []
    for pixel in pixels:
        r, g, b = extract_end_bits(3, pixel)
        hidden_pixels.append((
            int(r * 255 / 7),
            int(g * 255 / 7),
            int(b * 255 / 7)
        ))
